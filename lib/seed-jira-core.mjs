...
export async function runSeed(config, hooks = {}, { signal } = {}) {
  const {
    mode,
    dryRun,
    deleteArtifacts,
  ...
  const deleteSeeded = async (state) => {
    ...
    if (!deleteArtifacts) {
      ...
      return { deletedIssues: keys.length, artifactsDeleted: false };
    }
    ...
    return { deletedIssues: keys.length, artifactsDeleted: true };
  };
  ...
  const generate = async () => {
    ...
    return { createdCount, activityCount, sprintAssignCount, dryRun };
  };
